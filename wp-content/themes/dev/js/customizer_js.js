		...
		_init: function() {
            ...
            control.actions = function ($context) {
                if (control.params.live_title_id) {
                    ...
                    if (control.id === 'devfly_section_order_styling') {
    ...
    function _the_editor( container ){
        var _editor = {
            ready: function( container ) {
                ...
                control.container.addClass( 'devfly-editor-added' );
...
jQuery( window ).ready( function( $ ){
    ...
    if ( typeof devfly_customizer_settings !== "undefined" ) {
        if (devfly_customizer_settings.number_action > 0) {
            $('.control-section-themes h3.accordion-section-title').append('<a class="theme-action-count" href="' + devfly_customizer_settings.action_url + '">' + devfly_customizer_settings.number_action + '</a>');
        }
        if ( devfly_customizer_settings.is_plus_activated !== 'y' ) {
            $('#customize-info .accordion-section-title').append('<a target="_blank" style="text-transform: uppercase; background: #D54E21; color: #fff; font-size: 10px; line-height: 14px; padding: 2px 5px; display: inline-block;" href="http://devfloat.net">Contact our support</a>');
            $( '#accordion-section-devfly_order_styling > .accordion-section-title').append( '<span class="devfly-notice">Plus</span>' );
        }
    }
     ...
     */
    $( 'select[data-customize-setting-link="devfly_hero_layout"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';

        $( "li[id^='customize-control-devfly_hcl']").hide();
        $( "li[id^='customize-control-devfly_hcl"+v+"']").show();

    } );
    $( 'select[data-customize-setting-link="devfly_hero_layout"]').trigger( 'on_custom_load' );


    /**
     * For Gallery content settings
     */
    $( 'select[data-customize-setting-link="devfly_gallery_source"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';

        $( "li[id^='customize-control-devfly_gallery_source_']").hide();
        $( "li[id^='customize-control-devfly_gallery_api_']").hide();
        $( "li[id^='customize-control-devfly_gallery_settings_']").hide();
        $( "li[id^='customize-control-devfly_gallery_source_"+v+"']").show();
        $( "li[id^='customize-control-devfly_gallery_api_"+v+"']").show();
        $( "li[id^='customize-control-devfly_gallery_settings_"+v+"']").show();

    } );

    $( 'select[data-customize-setting-link="devfly_gallery_source"]').trigger( 'on_custom_load' );

    /**
     * For Gallery display settings
     */
    $( 'select[data-customize-setting-link="devfly_gallery_display"]').on( 'change on_custom_load', function(){
        var v = $( this).val() || '';
        switch ( v ) {
            case 'slider':
                $( "#customize-control-devfly_g_row_height, #customize-control-devfly_g_col, #customize-control-devfly_g_spacing").hide();
                break;
            case 'justified':
                $( "#customize-control-devfly_g_col, #customize-control-devfly_g_spacing").hide();
                $( "#customize-control-devfly_g_row_height").show();
                break;
            case 'carousel':
                $( "#customize-control-devfly_g_row_height, #customize-control-devfly_g_col").hide();
                $( "#customize-control-devfly_g_col, #customize-control-devfly_g_spacing").show();
                break;
            case 'masonry':
                $( "#customize-control-devfly_g_row_height").hide();
                $( "#customize-control-devfly_g_col, #customize-control-devfly_g_spacing").show();
                break;
            default:
                $( "#customize-control-devfly_g_row_height").hide();
    ...
    $( 'select[data-customize-setting-link="devfly_gallery_display"]').trigger( 'on_custom_load' );
