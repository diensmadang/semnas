...
class Themedavfly1_Misc_Control extends WP_Customize_Control {
...
class Themedavfly1_Textarea_Custom_Control extends WP_Customize_Control
{
	public function render_content() {
		?>
		<label>
...
class Saint_Theme_Support extends WP_Customize_Control {
	public function render_content() {
		echo wp_kses_post( 'Contact our support', 'devfly' );
	}
}
...
class themedavfly1_Editor_Custom_Control extends WP_Customize_Control
{
    /**
 ...
 */
class devfly_main_Alpha_Color_Control extends WP_Customize_Control {
 ...
 * @access public
 */
class Themedavfly1_Customize_Repeatable_Control extends WP_Customize_Control {
    ...
    public function __construct( $manager, $id, $args = array() )
    {
        ...
        if ( ! isset( $args['allow_unlimited'] ) || $args['allow_unlimited'] != false ) {
            $this->max_item =  apply_filters( 'devfly_reepeatable_max_item', absint( $args['max_item'] ) );
        }  else {
            $this->max_item = absint( $args['max_item'] );
        }

        $this->changeable =  isset(  $args['changeable'] ) && $args['changeable'] == 'no' ? 'no' : 'yes';
        $this->default_empty_title =  isset(  $args['default_empty_title'] ) && $args['default_empty_title'] != '' ? $args['default_empty_title'] : esc_html__( 'Item', 'devfly' );
    ...
    public function render_content() {
        ...
        <div class="repeatable-actions">
            <span class="button-secondary add-new-repeat-item"><?php _e( 'Add an item', 'devfly' ); ?></span>
        </div>
        <?php
    }
            ...
            <div class="widget">
                <div class="widget-top">
                    ...
                    <div class="widget-title">
                        <h4 class="live-title"><?php _e( 'Item', 'devfly' ); ?></h4>
                    </div>
                </div>
                                ...
                                <# if ( field.type ){ #>
                                    ...
                                    #>
                                        ...
                                        <# } else if ( field.type == 'media' ) { #>
                                            ...
                                            <div class="actions">
                                                <button class="button remove-button " <# if ( ! field.value.url ){ #> style="display:none"; <# } #> type="button"><?php _e( 'Remove', 'devfly' ) ?></button>
                                                <button class="button upload-button" data-media="{{field.media}}" data-add-txt="<?php esc_attr_e( 'Add', 'devfly' ); ?>" data-change-txt="<?php esc_attr_e( 'Change', 'devfly' ); ?>" type="button"><# if ( ! field.value.url  ){ #> <?php _e( 'Add', 'devfly' ); ?> <# } else { #> <?php _e( 'Change', 'devfly' ); ?> <# } #> </button>
                                        ...
                                        <# }  else if ( field.type == 'icon'  ) { #>
                                            ...
                                            <a href="#" class="remove-icon"><?php esc_html_e( 'Remove', 'devfly' ); ?></a>
                                        <# }  #>
                            ...
                            <div class="widget-control-actions">
                                <div class="alignleft">
                                    <span class="remove-btn-wrapper">
                                        <a href="#" class="repeat-control-remove" title=""><?php _e( 'Remove', 'devfly' ); ?></a> |
                                    </span>
                                    <a href="#" class="repeat-control-close"><?php _e( 'Close', 'devfly' ); ?></a>
                                </div>
                                <br class="clear">
                            </div>
...
/*
function devfly_enqueue_editor(){
...
*/
function devfly_customizer_control_scripts(){
    wp_enqueue_media();
    wp_enqueue_script( 'jquery-ui-sortable' );
    wp_enqueue_script( 'wp-color-picker' );
    wp_enqueue_style( 'wp-color-picker' );

    wp_enqueue_script( 'devfly-customizer', get_template_directory_uri() . '/js/customizer_js.js', array( 'customize-controls', 'wp-color-picker' ) );
    wp_enqueue_style( 'devfly-customizer',  get_template_directory_uri() . '/css/customizer.css' );

}

add_action( 'customize_controls_enqueue_scripts', 'devfly_customizer_control_scripts', 99 );